    ...
    <div class="container my-5">
        ...
        <div class="row">
            <div class="col-md-6">
                ...
                <img src="{{ asset('images/esdeveniments/' . $esdeveniment->imatge) }}" alt="{{ $esdeveniment->nom }}"
            ...
            <div class="col-md-6">
                ...
                @endphp
                <p class="text-light"><strong>Entrades disponibles:</strong>
                    {{ $entradesDisponibles > 0 ? $entradesDisponibles : 'No queden entrades disponibles' }}</p>
                ...
                @if (Auth::check() && Auth::user()->role === 'admin')
                    <form action="{{ route('esdeveniments.destroy', $esdeveniment->id) }}" method="POST" class="d-inline"
                        onsubmit="return confirm('Segur que vols eliminar aquest esdeveniment?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger d-inline">Eliminar</button>
                    </form>
                    <a href="{{ route('esdeveniments.edit', $esdeveniment->id) }}"
